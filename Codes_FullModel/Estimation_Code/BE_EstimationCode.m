...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
path_Plots = '../Figures/Estimation_Plots';
path_SetupCode  = '../Setup_Code/';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if exist(fullfile(path_SetupCode, 'Initial_Paramters.mat'), 'file')==0
    run(sprintf(path_SetupCode + 'BE_Datasetup.m'));
    load(fullfile(path_SetupCode, 'Initial_Paramters.mat'));
else
    load(fullfile(path_SetupCode, 'Initial_Paramters.mat'));
end
...
Pars_UB = [0.400, 0.250, 200.000, 0.100, 0.99, 0.900];
...
for i=1:NoOfIterations
    ...
    Optimal_Parameters(:,i) = particleswarm(@(pars) Objective_Avec(pars), length(Pars_LB), Pars_LB, Pars_UB, Options);  
    disp(Optimal_Parameters(:,i)) 
    [SS,XP0,XN0,D,SA,SR,L,tau,alpha] = Objective_Avec(Optimal_Parameters(:,i)');
    Value_Function(:,i)= SS;
    figure
    subplot(2,2,1)
